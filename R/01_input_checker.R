...
input_checker_MRCO <- function(metadata = NULL,
    ...
    # prepare return vars
    return_vars <- list("metadata" = NULL,
                        "cm" = NULL)
    ...
    return_vars$cm <-
        input_check_clustermatrix_MRCO(metadata = return_vars$metadata,
    ...
    # Edge filter:
    ## numerical:
    if (!is.numeric(edge_num_size_filter))
        stop("Expected input class for edge_num_size_filter is numeric")
    if (edge_num_size_filter >= nrow(metadata))
        stop(
            "Given numerical size filter for edges is larger than available nr of cells in input metadata, please reduce filter value."
        )
    if (edge_num_size_filter < 0)
        stop(
            "Numerical size filter for edges must be larger or equal to zero, please raise filter value."
        )

    ## proportional:
    if (!is.numeric(edge_prop_size_filter))
        stop("Expected input class for edge_prop_size_filter is numeric")
    if (edge_prop_size_filter >= 1)
        stop(
            "Given proportional size filter for edges is larger or equal to one, please reduce filter value."
        )
    if (edge_prop_size_filter < 0)
        stop(
            "Proportional size filter for edges must be larger or equal to zero, please raise filter value."
        )


    ## Other Arguments, Simple Checks
    if (!(is.logical(suggest_cut))) {
        stop("Expected input class for suggest_cut ",
             "is logical.")
    }
    if (!(is.logical(merge_downwards))) {
        stop("Expected input class for ",
             "merge_downwards is logical.")
    }
    if (!(is.logical(silent)))
        stop("Expected input class for silent is logical.")
    if (!is.logical(plot))
        stop("User argument plot must be of class logical.")
    ...
    if (is.list(nodes_selection)) {
        ...
        nodes_selection <-
            lapply(seq_along(nodes_selection), function(l) {
...
#' @import tibble rlang dplyr
#'
input_check_metadata_MRCO <- function(metadata = NULL,
                                      metadata_column_name = NULL,
                                      nbins = NULL) {
    if (!is.numeric(nbins)) {
        stop(
            "Unsupported nbins argument format. ",
            "Must be numeric, user supplied: ",
            class(nbins)
        )
    }
    if (!is.data.frame(metadata))
        stop("Unsupported metadata argument format.
    ...
    if (is.null(rownames(metadata))) {
        ...
        metadata <-
            metadata %>% tibble::rownames_to_column(var = "cell")
    }
    ...
    if (!quo_is_null(metadata_column_name)) {
        ...
        if (is.numeric(pull(metadata, !!metadata_column_name))) {
            ...
            if (nbins > max_bins) {
                stop(
                    "Number of bins request by user: ",
                    nbins,
                    ", is larger than maximum available bins: ",
                    max_bins,
                    " from selected numeric variable ",
                    as_name(metadata_column_name),
                    "."
                )
            }
            if (nbins <= 0) {
...
#' @import tibble rlang dplyr
#'
input_check_clustermatrix_MRCO <- function(metadata = NULL,
                                           clustering_columns = NULL) {
    # clustering_columns <- enquo(clustering_columns)
    if (quo_is_null(clustering_columns)) {
        stop("Specifying clustering_columns is required to have",
             " safe data handling!")
    }
    ...
    if (quo_is_call(clustering_columns)) {
        ...
        cm <- metadata %>%
            dplyr::select(c(starts_with(as_name(
                clustering_columns
            )), -"cell"))%>%
            as.data.frame()
    }
