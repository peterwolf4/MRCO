...
create_node_branch_list_MRCO <- function(graph_layout = graph_layout,
  ...
  leaf_nodes <- graph_layout %>%
    filter(.data$resolution == max(.data$resolution)) %>% pull("id")
  res_steps <- graph_layout %>% select("resolution") %>%
    distinct() %>% pull()
  branch_str <- matrix("X_Y", nrow = length(res_steps), ncol = length(leaf_nodes))
  ...
  for (n in seq((length(res_steps)-1),1,-1)){
    ...
    to_edges <- edges %>% filter(.data$to %in% to_nodes) %>%  group_by(.data$to) %>%
        slice_max(order_by = .data$e_prop_size, n = 1, with_ties = FALSE)

    if ( any(is.na(match(to_nodes,to_edges$to)))){
        stop("Matching edges to node failed. Please report this bug!
